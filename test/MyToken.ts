...
describe("My Token", () => {
    ...
    describe("Transfer", () => {
        ...
        it ("shoud be reverted with insufficient balance error", async () => {
            const sigenr1 = signers[1];
            await expect(
                myTokenC.transfer(hre.ethers.parseUnits((MINTING_AMOUNT + 1n).toString(), DECIMALS), sigenr1.address)
    ...
    describe("TransferFrom", () =>  {
        ...
        it("homework2", async () => {
            ...
            expect(await myTokenC.balanceOf(sigenr0.address)).to.equal((MINTING_AMOUNT * 10n **DECIMALS) - amount) ;
